...
- No databases, Docker, or external service setup required. The Matrix homeserver is remote at `http://80.225.209.87:3338`.
...
- `vox send` may print `Room creation error: 'str' object has no attribute 'value'` — this is a known pre-existing issue in the Matrix backend's room creation logic; the CLI still reports success due to fallback handling.
